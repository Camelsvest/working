...
struct _netio_module_vtable_t {
    async_module_uninit_func_t  async_module_uninit_func;
...
static void netio_module_uninit(bus_module_t *module)
{
    ...
    if (module != NULL)
    {
        ...
        if (netio_module->_vptr->async_module_uninit_func != NULL)
            netio_module->_vptr->async_module_uninit_func(module);
        
    }
}

static netio_module_vtable_t netio_module_vtable = {
    .async_module_uninit_func = NULL,
...
static int32_t netio_module_init(netio_module_t *module, uint32_t id, const char *desc)
{
    ...
    if (module != NULL) {
        ...
        if (ret == 0)
        {
            ...
            module->_vptr->async_module_uninit_func = module->base._vptr->uninit_func;
...
void destroy_netio_module(netio_module_t *netio)
{
    if (netio != NULL)
    {
        ...
        if (netio->_vptr != NULL && netio->_vptr->netio_uninit_func != NULL)
